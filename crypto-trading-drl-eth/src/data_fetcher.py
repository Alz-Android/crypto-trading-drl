...
class CryptoCompareDataFetcher:
    ...
    def get_historical_hourly(self, fsym='ETH', tsym='USD', limit=2000, toTs=None):
        """
        Fetch hourly historical data for a cryptocurrency pair.

        Args:
            fsym (str): From symbol (e.g., 'ETH')
    ...
    def get_multiple_days(self, fsym='ETH', tsym='USD', days=365):
        """
    ...
    def save_data(self, df, filename, data_dir='data'):
        """
...
if __name__ == "__main__":
    ...
    # Fetch Ethereum data for 30 days
    eth_data = fetcher.get_multiple_days('ETH', 'USD', days=30)

    if eth_data is not None:
        print("Sample of fetched data:")
        print(eth_data.head())
        print(f"\nData shape: {eth_data.shape}")
        print(f"Date range: {eth_data.index.min()} to {eth_data.index.max()}")

        # Save data
        fetcher.save_data(eth_data, 'eth_usd_30d')